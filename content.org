...
** TODO Pipewire in Docker                                         :@devlog:
:PROPERTIES:
:export_file_name: pipewire_in_docker
:export_date: 2022-11-26
:END:

Pipewire is a graph-based multimedia processing engine that lets you handle audio + video in real time! I've had way too much playing with it recently, but spent longer than I care to admit spinning it up in an Ubuntu container.

Most of the examples I saw floating around were using systemd or Fedora, but my requirements were

1. Ubuntu 22.04
2. Processes run forked in the foreground
3. Built from the latest source
4. Drop-in replacement for PulseAudio

Side note: I spent some time tinkering with 18.04 LTS, which require either a PPA or building your buildtools from scratch (they require versions not available older Debian systems). I highly recommend the PPA if you head that route...

*** Frontmatter and dependencies

As with most containers, we first needed to define the front matter and install all Pipewire build / runtime dependencies. There are probably a few unecesary packages floating around here, but the goal of this spike wasn't to optimize the container's size.
...
Next step are to set the environment variables relevant to Pipewire. I like to do this after installing packages from apt so I don't have to re-install everything if I change one variable.

#+begin_src Dockerfile
ARG PW_VERSION=0.3.59
...
Here we're pulling the latest version (as of writing) and defining our build dir. We're building Pipewire in ~/root~ as ~root~ -- worst practice, but it's a spike.

Now that we've installed our deps and set the stage, we're ready to build Pipewire. Meson is Pipewire's build system of choice. I don't have much experience with it, but it was easy enough to work with.
...
Last step is to setup the dominos of entrypoint scripts.
...
I like to breakdown the entrypoint scripts and order them with a filename prefix. I forget exactly where I picked up this habbit, but it stuck a long time ago.
...
And that should do it! If we hop into the container and check Pulse utilities, we can see that our Pipewire server is running and properly emulating Pulse. Great success
